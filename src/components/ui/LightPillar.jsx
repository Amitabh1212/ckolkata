...
const LightPillar = ({
  ...
  useEffect(() => {
    ...
    else if (!isMobile && navigator.hardwareConcurrency && navigator.hardwareConcurrency <= 4 && quality === 'high') effectiveQuality = 'medium';

    const qualitySettings = {
      low:    { iterations: 24, waveIterations: 1, pixelRatio: Math.min(window.devicePixelRatio, 1.5), precision: 'mediump', stepMultiplier: 1.5 },
      medium: { iterations: 40, waveIterations: 2, pixelRatio: Math.min(window.devicePixelRatio, 1.5), precision: 'mediump', stepMultiplier: 1.2 },
    ...
    const initialize = () => {
      ...
      if (width === 0 || height === 0) return; // still no layout — ResizeObserver will retry
    ...
    // Try right away (works if layout is already committed)
    initialize();

    // Watch the container for its first non-zero size (handles SSR/hydration delay)
    const ro = new ResizeObserver((entries) => {
      for (const entry of entries) {
        const { width, height } = entry.contentRect;
        if (!initializedRef.current && width > 0 && height > 0) {
          initialize();
        } else if (initializedRef.current && rendererRef.current && materialRef.current) {
  ...
  useEffect(() => { if (materialRef.current) { const c = new THREE.Color(topColor); materialRef.current.uniforms.uTopColor.value = new THREE.Vector3(c.r, c.g, c.b); } }, [topColor]);
  useEffect(() => { if (materialRef.current) { const c = new THREE.Color(bottomColor); materialRef.current.uniforms.uBottomColor.value = new THREE.Vector3(c.r, c.g, c.b); } }, [bottomColor]);
